...
for iac in xrange(num_new_ac):
    for irt in xrange(num_routes):
        ...
        h_pt_temp = (10 * numpy.sin(numpy.pi * x_pt / (mrange/1e3))).tolist()
        h_pt.append(h_pt_temp)
...
while len(Aset) > 0 and _iter <= iter_max:
    ...
    subsystems.append(
        SerialSystem('allocation_param',
                     ...
                     output=True,
                     subsystems=[
                         IndVar('pax/flight', val=Aset[Fsub_i].pax_flt),
                         IndVar('flights/day', val=Aset[Fsub_i].flt_day),
                     ])
    )
                             ...
                             subsystems=[
                                 SerialSystem('bsplines',
                                              ...
                                              subsystems=[
                                                  ...
                                                  IndVar('h_pt', copy=copy, val=numpy.array(Aset[Fsub_i].h_pt)[copy], lower=0),
    ...
    if False:
        ...
        opt.add_design_variable('flights/day', lower=Aset[Fsub_i].flt_day_lb, upper=Aset[Fsub_i].flt_day_ub)
        ...
        opt('SNOPT')
        call(['mv', 'SNOPT_summary.out', folder_path+'/relaxed_SNOPT.out'])
        call(['mv', 'hist.hst', folder_path+'/relaxed_hist.hst'])
        call(['rm', 'SNOPT_print.out'])

    #Extarct the optimzation results
    h_pt = []
    for copy in xrange(num_routes*num_new_ac):
        h_pt_temp = main.vec['u'](('h_pt',copy)).tolist()
        h_pt.append(h_pt_temp)

    Aset[Fsub_i].h_pt = h_pt
